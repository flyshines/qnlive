package qingning.lecture.server.configuration;
...
@Configuration
@EnableWebMvc
@ComponentScan({"qingning.lecture.server.controller","qingning.server.advice"})
...
public class SystemConfiguration extends WebMvcConfigurerAdapter {
	...
	public ServiceManger getServiceManger() throws Exception{
		...
		serviceManger.initServer(new String[]{"classpath:qingning-shop-server.xml"});
	...
	public ClassPathXmlApplicationContext getApplicationContext(){
		if(context==null){
			context=new ClassPathXmlApplicationContext(new String[] {"shop-rpc-server.xml"});
			context.start();
		}
		return context;
	}
