package qingning.manage.server.configuration;
...
@Configuration
@EnableWebMvc
@ComponentScan({"qingning.manage.server.controller","qingning.server.advice"})
...
public class SystemConfiguration extends WebMvcConfigurerAdapter {
	...
	public ServiceManger getServiceManger() throws Exception{
		...
		serviceManger.initServer(new String[]{"classpath:qingning-manage-server.xml"});
	...
	public ClassPathXmlApplicationContext getApplicationContext(){
		if(context==null){
			context=new ClassPathXmlApplicationContext(new String[] {"manage-rpc-server.xml"});
			context.start();
		}
		return context;
	}
