...
public class ShopServerImpl extends AbstractQNLiveServer {
    ...
    public Map<String, Object> shopOpen(RequestEntity reqEntity) throws Exception {
        ...
        String shop_id = MiscUtils.getUUId();

        Map<String, Object> map = new HashMap<>();
        map.clear();
        map.put("shop_id", shop_id);
        this.readShop(shop_id, map,CommonReadOperation.CACHE_READ_SHOP, false,jedis);

        reqMap.put("room_address", MiscUtils.getConfigByKey("live_room_share_url_pre_fix") + shop_id);
        reqMap.put("room_id", shop_id);

        //1.缓存中读取店铺信息
        map.put(Constants.CACHED_KEY_SHOP, userId);
        String lectureLiveRoomKey = MiscUtils.getKeyOfCachedData(Constants.CACHED_KEY_SHOP, map);
        ...
        String user_role = getAccessInfoByToken(reqEntity.getAccessToken(),"user_role",jedis);

        reqMap.put("isLecturer", true);
        ...
        Map<String,String> userInfo = readUser(userId, queryOperation, readUserOperation, jedis);


        //readUser(userId, queryParam, readUserOperation, jedis);
        ...
        if(MiscUtils.isEmpty(reqMap.get("shop_name"))){
        	reqMap.put("shop_name", String.format(MiscUtils.getConfigByKey("shop.default.name"), userInfo.get("nick_name")));
        }
       shopModuleServer.createLiveRoom(reqMap);
        ...
        updateAccessInfoByToken(reqEntity.getAccessToken(),"user_role",user_role+","+Constants.USER_ROLE_LECTURER,jedis);
        //3.3增加讲师直播间信息缓存
    ...
    public Map<String, Object> updateLiveRoom(RequestEntity reqEntity) throws Exception {
        ...
        Jedis jedis = jedisUtils.getJedis();//获取jedis对象
        String liveRoomOwner = readLiveRoomInfoFromCached((String)reqMap.get("room_id"), "lecturer_id",
                reqEntity, readShopOperation, jedis, true);
        if (liveRoomOwner == null || !liveRoomOwner.equals(userId)) {
            throw new QNLiveException("100002");
        }
 
        //2.检测更新时间是否与系统一致
        String liveRoomUpdateTime = readLiveRoomInfoFromCached((String)reqMap.get("room_id"), "update_time",
                reqEntity, readShopOperation, jedis, true);
        ...
        //3.修改数据库
        reqMap.put("user_id", userId);
        Map<String, Object> dbResultMap = shopModuleServer.updateLiveRoom(reqMap);
...
//        if(.equals(Constants.HEADER_APP_NAME)){
            ...
            resultMap.put("binding_service_url", shopModuleServer.findCustomerServiceBySystemConfig(map).get("config_value"));
        //}
        resultMap.put("qr_code",getQrCode(userId,jedis));
        ...
        Map<String,String> lectureInfo = readLecturer(userId, request, readShopOperation, jedis);
        Map<String,Object> userInfo = shopModuleServer.findUserInfoByUserId(userId);
        ...
        if(queryType.equals("0")){
            if(jedis.exists(liveRoomListKey)){
                ...
                if(CollectionUtils.isEmpty(liveRoomsMap)){
                    ...
                    for(String roomIdCache : liveRoomsMap.keySet()){
                        Map<String,String> liveRoomMap = readLiveRoom(roomIdCache, reqEntity, readShopOperation, jedis, true);
                        ...
                        if(! CollectionUtils.isEmpty(liveRoomMap)){
                        	...
                        	RequestEntity entity = this.generateRequestEntity(null, null, CommonReadOperation.SYS_READ_LAST_COURSE, query);
                        	Map<String,String> courseInfo = readLastCourseOfTheRoom(roomId, lectureId, entity, readCourseOperation, jedis);
                ...
                List<Map<String, Object>> liveRoomByLectureId = shopModuleServer.findLiveRoomByLectureId(userId);
                List<Map<String,Object>> liveRoomListResult = new ArrayList<>();
            ...
            Map<String,String> liveRoomMap = readLiveRoom(reqMap.get("room_id").toString(), reqEntity, readShopOperation, jedis, true);
            if(CollectionUtils.isEmpty(liveRoomMap)){
                throw new QNLiveException("100002");
            }
        	String roomId = liveRoomMap.get("room_id");
...
//     */
    ...
    private Map<String,Object> getWeCatTemplateInfo(Map<String, Object> reqMap){
        ...
        Map<String,Object> templateInfo  = shopModuleServer.findServiceTemplateInfoByLecturerId(paramMap);//查找微信推送模板
        ...
        if (MiscUtils.isEmpty(templateInfo)) {//为空
            ...
            if (errcode != null && Integer.parseInt(errcode.toString()) != 0) {//创建失败
                ...
                shopModuleServer.insertServiceTemplateInfo(paramMap);
            }
        } else {
    ...
    public String getWeServiceNo(Map<String, String> serviceNoMap, String userId, String serviceNoKey, Jedis jedis){
        ...
        if (nowTimeStamp-expiresTimeStamp > 0) {  //accessToken已经过期了
            ...
            if (errCode != null ) {
                ...
                shopModuleServer.updateServiceNoInfo(authInfoMap);
...
//        return serviceNoMap.get("authorizer_access_token");
    ...
    public Map<String, Object> getCourseDetail(RequestEntity reqEntity) throws Exception {
        ...
        Jedis jedis = jedisUtils.getJedis();//获取jedis对象
        String userId = AccessTokenUtil.getUserIdFromAccessToken(reqEntity.getAccessToken());
        Map<String ,String> courseInfoMap = readCourse(course_id,reqEntity,readCourseOperation, jedis,false);
        ...
        List<String> latestStudentAvatarAddList = shopModuleServer.findLatestStudentAvatarAddList(queryMap);
        ...
        //分享URL
        //TODO 分享URL
        resultMap.put("share_url",getCourseShareURL(userId, course_id, courseInfoMap,jedis));
    ...
    public Map<String, Object> updateCourse(RequestEntity reqEntity) throws Exception {
        ...
        Map<String, String> course = readCourse((String)course_id, generateRequestEntity(null, null, null, reqMap), readCourseOperation, jedis, false);//课程信息
        ...
        if ("2".equals(newStatus)) {
            ...
            Map<String, Object> userInfo = shopModuleServer.findUserInfoByUserId(courseOwner);
            Map<String,Object> startLecturerMessageInformation = new HashMap<>();
            ...
            IMMsgUtil.sendMessageInIM(mGroupId, startLecturerMessageInformationContent, "", shopModuleServer.findLoginInfoByUserId(courseOwner).get("m_user_id").toString());//发送信息
...
//					//添加到老师发送的集合中
        ...
        if(!"2".equals(newStatus)){
            ...
            if (reqMap.get("start_time") != null) { //开课时间修改
                ...
                course = readCourse(course_id, generateRequestEntity(null, null, null, query), readCourseOperation, jedis, true);
                ...
                //TODO 推送多人  报名的所有人
                List<Map<String, Object>> userInfo = shopModuleServer.findCourseStudentListWithLoginInfo(course_id);
                ...
                Map<String, String> serviceNoMap = jedis.hgetAll(serviceNoKey);
                if (MiscUtils.isEmpty(serviceNoMap)) {
                    Map<String, Object> serviceNoMapObj = shopModuleServer.findServiceNoInfoByLecturerId(userId);
                    MiscUtils.converObjectMapToStringMap(serviceNoMapObj, serviceNoMap);
                }
...
//                    }
        ...
        List<Map<String, Object>> courseGuestByCourseId = shopModuleServer.findCourseGuestByCourseId(course_id);
        for(Map<String, Object> courseGuest : courseGuestByCourseId){
            map.clear();
            map.put(Constants.GUEST_ID, courseGuest.get("user_id"));
    ...
    public Map<String, Object> getCourseList(RequestEntity reqEntity) throws Exception {
        ...
        Map <String,Object> resultMap = new HashMap<>();
        Map<String, Object> platformCourses = getPlatformCourses(pageCount, course_id, lecturerId);
     ...
     */
    @SuppressWarnings({ "unchecked"})
    private  Map<String, Object> getPlatformCourses(int pageCount,String courseId,String lecture_id) throws Exception{
        Jedis jedis = jedisUtils.getJedis();//获取jedis对象
    ...
    public Map<String, Object> createCoursePPTs(RequestEntity reqEntity) throws Exception {
        ...
        if(jedis.exists(courseKey)){
            ...
            //4.1检查课程是否存在
            Map<String,Object> courseMap = shopModuleServer.findCourseByCourseId(reqMap.get("course_id").toString());
            ...
            shopModuleServer.deletePPTByCourseId(reqMap.get("course_id").toString());
            ...
            if(pptList.size() > 0){
                ...
                shopModuleServer.createCoursePPTs(insertMap);
            }
        }
        jedis.sadd(Constants.CACHED_UPDATE_LECTURER_KEY, userId);
        return resultMap;
    ...
    public Map<String, Object> getCourseStudentList(RequestEntity reqEntity) throws Exception {
    	...
    	if(StringUtils.isBlank(studentNum)){
    		Map<String,String> courseMap = readCourse((String)reqMap.get("course_id"),
    	...
    	if( MiscUtils.isEmpty(reqMap.get("data_source")) || "1".equals(reqMap.get("data_source"))){
    		...
    		if(banUserIdList != null && banUserIdList.size() > 0){    			
    			...
    			banUserList = shopModuleServer.findBanUserListInfo(paraMap);
    	...
    	if(studentList.size()< pageCount){
    		...
    		List<Map<String,Object>> studentListDB = shopModuleServer.findCourseStudentList(queryMap);
    ...
    public Map<String, Object> getCourseProfitList(RequestEntity reqEntity) throws Exception {
        ...
        //查询直播间信息
        Map<String,String> roomInfo = readLiveRoom(room_id,reqEntity,readShopOperation,jedis,false);
        if(roomInfo == null){
            throw new QNLiveException("100002");
        }
        ...
        List<Map<String,Object>> list = shopModuleServer.findCourseProfitList(reqMap);
    ...
    public Map<String, String> getDistributionInfo(RequestEntity reqEntity) throws Exception {
        ...
        Map<String,String> values = readLecturer(userId, reqEntity, readShopOperation, jedis);
        return values;
    }
    ...
    public Map<String, Object> getRoomDistributerInfo(RequestEntity reqEntity) throws Exception {
        ...
        String liveRoomOwner =  readLiveRoomInfoFromCached(room_id, "lecturer_id", reqEntity, readShopOperation, jedis,true);
        if (liveRoomOwner == null || !liveRoomOwner.equals(userId)) {
            throw new QNLiveException("100002");
        }
        ...
        long distributer_num = MiscUtils.convertObjectToLong(readLiveRoomInfoFromCached(room_id, "distributer_num", reqEntity, readShopOperation, jedis,true));
        Map<String, Object> result = new HashMap<String,Object>();
        result.put("distributer_num", distributer_num);
        List<Map<String,Object>> list = null;
        if(distributer_num>0){
        	list = shopModuleServer.findDistributionRoomByLectureInfo(reqMap);
        	if(!MiscUtils.isEmpty(list)){
        		...
        		for(Map<String,Object> values:list){
        			...
        			Map<String,String> userInfo = readUser(distributerId, entity, readUserOperation, jedis);
    ...
    public Map<String, Object> getRoomDistributerCoursesInfo(RequestEntity reqEntity) throws Exception {
        ...
        for(String valueStr:courseList){
            resultList.add(convertCachedStringToMap(valueStr));
        }
        Map<String,Object> result = new HashMap<String,Object>();
    ...
    public Map<String, Object> createRoomShareCode(RequestEntity reqEntity) throws Exception {
        ...
        shopModuleServer.insertLecturerDistributionLink(reqMap);
        ...
        Map<String,String> userMap = readUser(userId, queryOperation, readUserOperation, jedis);
        reqMap.clear();
    ...
    /*   @SuppressWarnings("unchecked")
       ...
       public void createRoomDistributer(RequestEntity reqEntity) throws Exception {
           ...
           map.put(Constants.FIELD_ROOM_ID, room_id);
           String liveRoomKey = MiscUtils.getKeyOfCachedData(Constants.CACHED_KEY_ROOM, map);
           String liveRoomOwner = values.get("lecturer_id");//readLiveRoomInfoFromCached(room_id, "lecturer_id", reqEntity, readShopOperation, jedisUtils,true);
           ...
           Map<String,String> distributerRoom = readDistributerRoom(userId, room_id, readRoomDistributerOperation, jedis);
           if(!MiscUtils.isEmpty(distributerRoom)){
               throw new QNLiveException("100027");
           }
           Map<String,String> query = new HashMap<String,String>();
           query.put("distributer_id", userId);
           Map<String,String> distributer = readDistributer(userId,
                   generateRequestEntity(null, null, null, query), readDistributerOperation, jedis, true);
           if(MiscUtils.isEmpty(distributer)){
               values.put(CommonReadOperation.SYS_INSERT_DISTRIBUTER, "1");
           }
           values.put("distributer_id", userId);
           String newRqCode = MiscUtils.getUUId();
           values.put("newRqCode",newRqCode);

           Map<String,String> roomInfo =  readDistributerRoom(userId, room_id, readRoomDistributerOperation, jedis,true);
           ...
           Map<String,Object> insertResultMap = shopModuleServer.createRoomDistributer(values);
           query.clear();
           ...
           distributerRoom = readDistributerRoom(userId, room_id, readRoomDistributerOperation, jedis);
           ...
           if(totaldistributerAdd){
               ...
               if(!jedis.exists(lecturerKey)){
                   readLecturer(userId, generateRequestEntity(null, null, null, map), readShopOperation, jedis);
               }
        ...
        String roomName = readLiveRoomInfoFromCached(room_id, "room_name", reqEntity, readShopOperation, jedis,true);
    ...
    */
    ...
    public Map<String, Object> getCourseStatistics(RequestEntity reqEntity) throws Exception {
        ...
        //数据统计
        List<Map<String,String>> courseList = getPlatformCourses(userId,(int)reqMap.get("page_count"),(String)reqMap.get("course_id"));
    ...
    public Map<String,Object> getFanList(RequestEntity reqEntity) throws Exception{
        ...
        map.put(Constants.FIELD_ROOM_ID, room_id);
        String liveRoomOwner = readLiveRoomInfoFromCached(room_id, "lecturer_id", reqEntity, readShopOperation, jedis,true);
        String fans_num_str = readLiveRoomInfoFromCached(room_id, "fans_num", reqEntity, readShopOperation, jedis,true);
        if (liveRoomOwner == null || !liveRoomOwner.equals(userId)) {
            throw new QNLiveException("100002");
        }
        ...
        if(!MiscUtils.isEmpty(fans_num_str) && Long.parseLong(fans_num_str)>0){
            ...
            List<Map<String,Object>> list = shopModuleServer.findRoomFanList(map);
...
//        String expires_in = (String) reqMap.get("expires_in");
        ...
        if (typeInfo.getString("id").equals("2") && !verifyInfo.getString("id").equals("-1")) {
            ...
            Map<String, Object> oldServiceInfo = shopModuleServer.findServiceNoInfoByAppid(authorizer_appid);
            //存储公众号的授权信息
            Map<String, String> authInfoMap = new HashMap<>();
            ...
            if (!MiscUtils.isEmpty(oldServiceInfo)) { //更新授权信息 重复授权 token会失效
                ...
                shopModuleServer.insertServiceNoInfo(authInfoMap);
            }
            //重定向成功页面 然后扫码登录 绑定直播间
            result.put("redirectUrl", MiscUtils.getConfigByKey("weixin_pc_no_login_qr_url").replace("APPID", authorizer_appid).replace("", URLEncoder.encode(authauthorizer_info_base.getString("nick_name"), "utf-8")));
    ...
    public void bindingRoom(RequestEntity reqEntity) throws Exception {
        ...
        int count = shopModuleServer.updateServiceNoLecturerId(paramMap);
        if (count < 1) {
            throw new QNLiveException("150001");
        }
        ...
        Jedis jedis = jedisUtils.getJedis();//获取jedis对象
        Map<String, Object> authInfoMap = shopModuleServer.findServiceNoInfoByAppid(appid);
    ...
    private List<Map<String,String>> getCourseList(String lecturerId,int pageCount,String course_id,int courceStatus,Long queryTime,Long postion, boolean preDesc, boolean finDesc) throws Exception {
        ...
        Map<String,String> queryParam = new HashMap<String,String>();
        queryParam.put("course_id", course_id);
        Map<String, String> courseInfoMap = readCourse(course_id,
     ...
     */
    @SuppressWarnings({ "unchecked"})
    private   List<Map<String,String>> getPlatformCourses(String userId,int pageCount,String courseId) throws Exception{
        Jedis jedis = jedisUtils.getJedis();//获取jedis对象
        long currentTime = System.currentTimeMillis();//当前时间
        int offset = 0;//偏移值
        Set<String> courseIdSet;//查询的课程idset
        ...
        if(courseId == null || courseId.equals("")){
            ...
            Map<String, String> courseMap = readCourse(courseId, this.generateRequestEntity(null, null, null, param), readCourseOperation, jedis, true);//从缓存中读取课程信息
        ...
        ((JedisBatchCallback)jedis).invoke(new JedisBatchOperation(){
            ...
            public void batchOperation(Pipeline pipeline, Jedis jedis) {
                ...
                if(MiscUtils.isEmpty(course_num)){                    
                    Map<String,Object> distributeInfo = shopModuleServer.findDistributerInfo(query);
                ...
                List<Map<String,Object>> list = shopModuleServer.findRoomDistributerCourseInfo(query);
                Date start_time = null;
                for(Map<String,Object> value : list){
                    String valueStr = convertMaptoCachedString(value);
    ...
    private List<Map<String,String>> getCourseOnlyFromCached(Jedis jedis, String key, Long startTime, Long postion, int pageCount, boolean desc){
        ...
        return readCourseListInfoOnlyFromCached(jedis, list,readCourseOperation);
    }
    ...
    public Map<String, Object> getCustomerService(RequestEntity reqEntity) throws Exception {
        ...
        Map<String,Object> customerQrCodeUrl = shopModuleServer.findCustomerServiceBySystemConfig(map);//获取客服二维码url
        retMap.put("customerQrCodeUrl",customerQrCodeUrl.get("config_value"));

        map.put("config_key","customerPhoneNum");
        Map<String,Object> customerPhoneNum = shopModuleServer.findCustomerServiceBySystemConfig(map);//获取客服电话
        retMap.put("customerPhoneNum",customerPhoneNum.get("config_value"));

        map.put("config_key","customerTitle");
        Map<String,Object> customerTitle = shopModuleServer.findCustomerServiceBySystemConfig(map);//标题
        retMap.put("customerTitle",customerTitle.get("config_value"));

        map.put("config_key","customerHint");
        Map<String,Object> cystomerHint = shopModuleServer.findCustomerServiceBySystemConfig(map);//客服二维码提示
...
//        }
        ...
        parameter.put("phone_number",phone);
        parameter.put("old_user",0);
        if(!CollectionUtils.isEmpty(shopModuleServer.findByPhone(parameter))){
            log.error("绑定手机号码>>>>手机号码已被绑定过");
            throw new QNLiveException("120023");
        }
        Map<String, Object> queryMap = new HashMap<>();
        queryMap.put("user_id", userId);
        Map<String, String> userMap = readUser(userId, this.generateRequestEntity(null, null, null, queryMap), readUserOperation, jedis);

        queryMap.put("phone_number", phone);

        queryMap.put("old_user", 1);
        Map<String, Object> oldUser = shopModuleServer.findByPhone(queryMap);


        if (!MiscUtils.isEmpty(oldUser)) {//有老用户
            Map<String, Object> newMap = new HashMap<String, Object>();
            newMap.put("user_id", oldUser.get("user_id"));
            newMap.put("update_time", new Date());
            newMap.put("old_user", 0);
            shopModuleServer.updateUser(newMap);

            Map<String, Object> newUserLogin = shopModuleServer.findLoginInfoByUserId(userId);
            if (!MiscUtils.isEmpty(newUserLogin)) {
                shopModuleServer.deleteUserAndLogin(userId);

                Map oldUserMap = new HashMap();
                //  oldUserMap.putAll(newUserLogin);
                oldUserMap.put("user_id", oldUser.get("user_id"));
                oldUserMap.put("update_time", new Date());
                oldUserMap.put("union_id", newUserLogin.get("union_id"));
                oldUserMap.put("web_openid", newUserLogin.get("web_openid"));
                oldUserMap.put("app_openid", newUserLogin.get("app_openid"));
                shopModuleServer.updateLoginInfo(oldUserMap);
                userId = oldUser.get("user_id").toString();

            }
            //删除旧用户token
            Map<String, Object> cacheKey = new HashMap<>();
            cacheKey.put(Constants.CACHED_KEY_ACCESS_TOKEN_FIELD, reqEntity.getAccessToken());
            String token = MiscUtils.getKeyOfCachedData(Constants.CACHED_KEY_ACCESS_TOKEN, cacheKey);
            jedis.del(token);

            cacheKey.put("user_id", oldUser.get("user_id"));
            String userKey = MiscUtils.getKeyOfCachedData(Constants.CACHED_KEY_USER, cacheKey);
            jedis.del(userKey);
...
//        }
        ...
        if (jedis.exists(lectureLiveRoomKey)) {//有直播间
            Map<String,String> userInfo = readUser(userId, reqEntity, readUserOperation, jedis);//查找当前用户是否有直播间
            if(MiscUtils.isEmpty(userInfo.get("phone_number"))){//如果没有
                updateUserPhone(phone,userId);
                jedis.del(key);
                readUser(userId, reqEntity, readUserOperation, jedis);
    ...
    public void updateUserPhone(String phone,String userid){
        ...
        shopModuleServer.updateUser(map);
        shopModuleServer.updateLoginInfo(map);
    }
    ...
    public Map<String, Object> delCourse(RequestEntity reqEntity) throws Exception {
        ...
        if(courseInfoMap.get("status").equals("2") || courseInfoMap.get("status").equals("1")){
            if(Long.valueOf(courseInfoMap.get("course_amount")) > 0 || Long.valueOf(courseInfoMap.get("extra_amount")) > 0){//判断当前课程是否产生收益
                ...
                shopModuleServer.updateCourse(course);//修改数据库数据
                //把删除课程存入缓存中用于统计
                ...
                infomation.put("course_id", reqMap.get("course_id").toString());
                infomation.put("creator_id", userId);
                infomation.put("message", MiscUtils.getConfigByKey("del_cource_message"));
                infomation.put("message_type", "1");
                ...
                //获取当前课程的学生
                List<Map<String, Object>> courseAllStudentList = shopModuleServer.findCourseAllStudentList(course_id);
...
//                    }
    ...
    public Map<String, Object> createSeries(RequestEntity reqEntity) throws Exception {
        ...
        if(query_type.equals("0")){
            ...
            Map<String,String> shopInfo = readShop((String)reqMap.get("shop_id"), reqMap,CommonReadOperation.CACHE_READ_SHOP, false,jedis);
        ...
        //3创建系列db
        Map<String, Object> dbSeries = shopModuleServer.createSeries(reqMap);

        //4把系列加入缓存
        Map<String, String> series = readSeries((String)dbSeries.get("series_id"),
    ...
    public Map<String, Object> updateSeries(RequestEntity reqEntity) throws Exception {
        ...
        Map<String, String> series = readSeries(series_id, generateRequestEntity(null, null, null, keyMap), readSeriesOperation, jedis, true);
        String lecturer_id = series.get("lecturer_id");
        if(!lecturer_id.equals(user_id)){
            throw new QNLiveException("210001");
        }
        Map<String, Object> resultMap = shopModuleServer.updateSeries(reqMap);
        jedis.del(key);//删除现在的缓存 进行更新
        Map<String, String> seriesInfoMap = readSeries(series_id, generateRequestEntity(null, null, null, reqMap), readSeriesOperation, jedis, true);
    ...
    public Map<String, Object> updown(RequestEntity reqEntity) throws Exception {
        ...
        if(query_type.equals("1")){//系列
            ...
            if(query_from.equals("0")){
                ...
                course  = readCourse(courseId,
                        generateRequestEntity(null, null, CommonReadOperation.SYS_READ_SAAS_COURSE, updownMap), readCourseOperation, jedis, true);
            }
            ...
            if(query_type.equals("0")){//单品
                ...
                if(updown.equals("1")){//往上架加入
                    ...
                    Map<String, Object> dbResultMap = shopModuleServer.increaseSeriesCourse(series_id);
                    /*
                     * 更新讲师所有上架系列课缓存
                     */
                    ...
                    Map<String, String> seriesMap = readSeries(series_id, readSeriesReqEntity, readSeriesOperation, jedis, true);
                    ...
                    shopModuleServer.delSeriesCourse(series_id);
                }
                jedis.del(seriesKey);
                //获取系列课程详情
    ...
    public Map<String, Object> updateSeriesCourse(RequestEntity reqEntity) throws Exception {
        ...
        map.put("course_id",course_id);
        String courseKey = MiscUtils.getKeyOfCachedData(Constants.CACHED_KEY_COURSE, map);
        Map<String, String> courseInfo = readCourse(course_id, generateRequestEntity(null, null, null, map), readCourseOperation, jedis, true);//获取课程信息
        if(MiscUtils.isEmpty(courseInfo)){
            courseInfo = readCourse(course_id, generateRequestEntity(null, null, CommonReadOperation.SYS_READ_SAAS_COURSE, map), readCourseOperation, jedis, true);//获取课程信息
        }
        ...
        if(query_type.equals("1")){//移出系列
            ...
            shopModuleServer.delSeriesCourse(oldSeriesId);
            String seriesKey = MiscUtils.getKeyOfCachedData(Constants.CACHED_KEY_SERIES, map);
            jedis.del(seriesKey);
            //获取系列课程详情
            Map<String, String> oldSeriesInfo = readSeries(oldSeriesId, generateRequestEntity(null, null, null, map), readSeriesOperation, jedis, true);
            ...
            if(series_course_type.equals("0")){
                resltMap = shopModuleServer.updateSeriesCourse(course);
            }else{
                resltMap = shopModuleServer.updateSaasSeriesCourse(course);
            }
            //删除系列缓存中的课程
            ...
            courseInfo = readCourse(course_id, generateRequestEntity(null, null, null, map), readCourseOperation, jedis, true);//获取课程信息
            if(MiscUtils.isEmpty(courseInfo)){
                courseInfo =  readCourse(course_id, generateRequestEntity(null, null, CommonReadOperation.SYS_READ_SAAS_COURSE, map), readCourseOperation, jedis, true);//获取课程信息
            }
            ...
            String series_id = reqMap.get("series_id").toString();
            map.put("series_id",series_id);
            Map<String, Object> stringObjectMap = shopModuleServer.increaseSeriesCourse(series_id);
            String seriesKey = MiscUtils.getKeyOfCachedData(Constants.CACHED_KEY_SERIES, map);
            jedis.del(seriesKey);
            //获取系列课程详情
    ...
    public Map<String, Object> updateSeriesCourseLonely(RequestEntity reqEntity) throws Exception {
        ...
        resltMap = shopModuleServer.updateCourseLonely(reqMap);
        jedis.del(courseKey);
    ...
    public Map<String, Object> findSeriesIncomeList(RequestEntity reqEntity) throws Exception {
         ...
         * 从缓存中获取店铺信息
         */
/*        Map<String, String> shopMap = readShop(shopId, reqEntity, readShopOperation, jedis);
 ...
 */       
         ...
         * 从缓存中获取系列课的详情
         */
        Map<String, String> seriesInfoMap = readSeries(seriesId, reqEntity, readSeriesOperation, jedis, true);
        ...
        List<Map<String, Object>> profitInfoList = shopModuleServer.findSeriesProfitListByMap(reqMap);
         ...
         * 下标为1表示总收入
         */
        List<Map<String, Object>> seriesProfitStatistics = shopModuleServer.findSeriesProfitStatistics(reqMap);
...
//        Set<Tuple> guestCourseSet = readCuestCourseIdSet(loginedUserId, nowScore, 1,
...
//        	nextStartCourseInfo = readCourse(nextCourseId,
...
//	        		List<Map<String, Object>> courseGuest = shopModuleServer.getCourseGuestByMap(selectCourseGuestMap);
...
//         * 查找登录用户作为讲师最近直播（时间往前找）的1门直播课程
//         */
//        lecturerCourseSet = readLecturerAllCourseIdSet(loginedUserId, nowScore, 1,
//        		readLecturerCourseReqEntity, readCourseOperation, jedis, false);
//        /*
//         * 查找登录用户作为嘉宾最近直播（时间往前找）的1门直播课程
//         */
//        guestCourseSet = readCuestCourseIdSet(loginedUserId, nowScore, 1,
...
//        		livingCourseTmp = readCourse(lecturerCourse.getElement(),
...
//        		livingCourseTmp = readCourse(guestCourse.getElement(),
//            			this.generateRequestEntity(null, null, null, readCourseMap),
//            			readCourseOperation, jedis, true);
//            	if (!MiscUtils.isEmpty(livingCourseTmp)) {
...
//    }
    ...
    public Map<String, Object> getSingleLecturerLiveCourse(RequestEntity reqEntity) throws Exception {
        ...
        if(!MiscUtils.isEmpty(previewCourseIdSet) || !MiscUtils.isEmpty(guestPreviewCourseIdSet)){
            ...
            if(!MiscUtils.isEmpty(previewCourseIdSet)){
                ...
                map.clear();
                map.put("course_id",courseId);
                lecturerCourseMap = readCourse(courseId, generateRequestEntity(null, null, null, map), readCourseOperation, jedis, true);
                startTime  = Long.valueOf(lecturerCourseMap.get("start_time"));
            }
            ...
            if(!MiscUtils.isEmpty(guestPreviewCourseIdSet)){
                ...
                map.clear();
                map.put("course_id",courseId);
                guestCourseMap = readCourse(courseId, generateRequestEntity(null, null, null, map), readCourseOperation, jedis, true);
                map.put("user_id",lecturer_id);
                Map<String, Object> courseGuestByUserAndCourse = shopModuleServer.findCourseGuestByUserAndCourse(map);
     ...
     */
    @SuppressWarnings("unchecked")
    @FunctionName("guestCourses")
    public Map<String, Object> guestCourses(RequestEntity reqEntity) throws Exception {
        Map<String, Object> reqMap = (Map<String, Object>) reqEntity.getParam();

        Jedis jedis = jedisUtils.getJedis();
        String guestUserId = AccessTokenUtil.getUserIdFromAccessToken(reqEntity.getAccessToken());//邀请人 通过token获取 先判断是否是讲师
        Long currentTime = System.currentTimeMillis();
        String courseId = null;
        Integer pageCount = Integer.valueOf(reqMap.get("page_count").toString());
        Integer status = 4;
        if(!MiscUtils.isEmpty(reqMap.get("course_id"))){
            courseId = reqMap.get("course_id").toString();
            Map<String,Object> map = new HashMap<>();
            map.put("course_id",courseId);
            ...
            MiscUtils.courseTranferState(currentTime, courseInfoMap);//判断课程状态
            status = Integer.valueOf(courseInfoMap.get("status"));
        }
        ...
        List<Map<String, Object>> guestCourses = shopModuleServer.findGuestCourses(queryMap);
       ...
       //     List<Map<String, Object>> guestCourses = shopModuleServer.findGuestCourses(queryMap);
        ...
        if(status == 4){
            ...
            if(MiscUtils.isEmpty(courseId)){//如果没有传入courceid 那么就是最开始的查询  进行倒叙查询 查询现在的
                ...
                startIndex = "("+courseScoreByRedis;//设置起始位置 '(' 是要求大于这个参数
                endIndex =MiscUtils.convertInfoToPostion(currentTime,0L)+"";//设置结束位置
            }
            ...
            pageCount =  pageCount - courseIdSet.size();//用展示数量减去获取的数量  查看是否获取到了足够的课程数
            if( pageCount > 0){//如果返回的值不够
                courseId = null;//把课程id设置为null  用来在下面的代码中进行判断
                status = 1;//设置查询课程状态 为结束课程 因为查找出来的正在直播和预告的课程不够数量
            }
        }
        ...
        if(courseIdList.size() > 0){
          ...
          //      shopModuleServer.findGuestCourses()
                Map<String, String> courseInfoMap = readCourse(course_id, this.generateRequestEntity(null, null, null, queryParam), readCourseOperation, jedis, true);//从缓存中读取课程信息
                if(!MiscUtils.isEmpty(courseInfoMap)){
                    MiscUtils.courseTranferState(currentTime, courseInfoMap);//进行课程时间判断,如果课程开始时间大于当前时间 并不是已结束的课程  那么就更改课程的状态 改为正在直播
