package qingning.saas.server.configuration;
...
@Configuration
@EnableWebMvc
@ComponentScan({"qingning.saas.server.controller","qingning.server.advice"})
...
public class SystemConfiguration extends WebMvcConfigurerAdapter {
	...
	public ServiceManger getServiceManger() throws Exception{
		...
		serviceManger.initServer(new String[]{"classpath:qingning-saas-server.xml"});
	...
	public ClassPathXmlApplicationContext getApplicationContext(){
		if(context==null){
			context=new ClassPathXmlApplicationContext(new String[] {"saas-rpc-server.xml"});
			context.start();
		}
		return context;
	}
